...
def change_setpoint(sig, frame):
    global SET_POINT
    print("\n")
    new_x = input("Enter new setpoint for X: ")
    new_y = input("Enter new setpoint for Y: ")

    try:
        SET_POINT = [int(new_x), int(new_y)]
    except:
        print("Failed to set new setpoint")
...
if __name__ == "__main__":
    ...
    # Register the signal handler function for the SIGINT signal (CTRL+C)
    # signal.signal(signal.SIGINT, change_setpoint)

    # Create a new thread to run the control loop
